...
- The AI feature (`AIClient`, `AIProfiles`, `AIWorkspace`) is independent of the backend. It streams from OpenAI-compatible `/chat/completions` endpoints. It stores one JSON profile per normalized base URL under `LocalRecorder/AIProfiles`, API keys in the Keychain keyed by endpoint, and the current selection in UserDefaults.
...
- `core.py`:
  ...
  - `Segmenter` groups 20 ms frames with VAD into segments. Pre-roll is used only after silence. A segment is finalized after `endpoint_silence_ms` of silence or force-cut at `max_segment_seconds`. Final segments are contiguous and non-overlapping by design: there is no text dedup, so repeated words the speaker really said are kept. Don't add overlap or dedup. Finals cut at `max_segment_seconds` carry `forced_cut: true`.
...
- `translation.py`:
  - `TranslationPlanner` turns finals into translation units. A silence-ended final is one unit. After a forced cut, the last sentence carries into the next final with its trailing punctuation removed, because Qwen3-ASR punctuates cut-off audio as if the sentence had ended.
...
- Ordinary load, transcription, and translation stay offline; only an explicit download or AI request uses the network.
