...
- ASR `api_protocol: qwen_realtime` uses `asr_bridge.py` to forward bounded, base64 PCM chunks through `asr_api_audio` events with a `call_id` and final `done` marker. `AppModel` assembles one request, and `QwenRealtimeASRClient` performs native WSS requests using Keychain credentials. It confirms `turn_detection: null`, appends PCM, commits once, and reads only input-ASR events; it never sends `response.create`. The frontend returns `asr_api_result` (same call ID, text or error). The backend worker waits for this result before emitting the normal final and finishing the session. Qwen uses fixed local pause detection and suppresses paid rolling previews. `scripts/test_qwen_asr.sh` tests the native protocol with a local fixture; `test_core` tests the socket bridge and final draining.
